...
def roll_dice(num_rolls, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    total=0
    flag = False#判断有没有出现点数为1的骰子
    for _ in range(num_rolls):#投n次
        roll = dice()#每次的点数
        total += roll
        if roll == 1:
            flag = True
...
def free_bacon(score):
    ...
    # Trim pi to only (score + 1) digit(s) 把pi截断到score+1位？
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    pi =pi//(10**(100-score))%10
    return pi + 3
    # END PROBLEM 2
...
def pig_pass(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4b
    "*** YOUR CODE HERE ***"
    return player_score < opponent_score and 0 < opponent_score - player_score < 3
    # END PROBLEM 4b
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            if not extra_turn(score0,score1):##需要进行extra_turn时 返回True who应该不变 否则 who取反
                who=other (who)
        else:
            score1+=take_turn(strategy1(score1, score0),score0,dice)
            if not extra_turn(score1,score0):##需要进行extra_turn时 返回True who应该不变 否则 who取反
                who=other (who)
        if say is not None:
...
def make_averaged(original_function, trials_count=1000):
    ...
    def averaged_function(*args):
        ...
        for _ in range(trials_count):
            total += original_function(*args)#以args里的数据为参数调用一千次传入的函数
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    best_num_rolls = 1
    #初始化
    for num_rolls in range(1, 11): #计算每回合投1-10个骰子时的平均得分
        average_roll = make_averaged(roll_dice, trials_count)(num_rolls, dice)
        if average_roll > max_average_score:
            max_average_score = average_roll
...
def bacon_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    # 获取通过 Free Bacon 规则可以获得的分数
    free_bacon_score = free_bacon(opponent_score)
    score+=free_bacon_score#不用一下score上面报未存取运行不了 我也不知道咋办 也不能改函数参数
    # 如果 Free Bacon 给的分数 >= cutoff，选择不投骰子
    if free_bacon_score >= cutoff:
        return 0  # 投 0 个骰子，使用 Free Bacon
    # 否则，选择投 num_rolls 个骰子
    return num_rolls
    # END PROBLEM 10
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    # 计算通过 Free Bacon 策略获得的分数
    free_bacon_score = free_bacon(opponent_score)
    score += free_bacon_score
    # 如果 Free Bacon 给的分数 >= cutoff，选择投0个骰子
    if free_bacon_score >= cutoff:
        return 0  # 选择投0个骰子，使用 Free Bacon 策略
    # 判断是否触发额外回合
    if extra_turn(score, opponent_score):
        return 0  # 如果触发额外回合 选择投0个骰子
    # 如果没有触发额外回合，选择投num_rolls个骰子
    return num_rolls
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    优先考虑触发free bacon会不会获得更高分数
    如果有机会触发 swine align 或 pig pass，就不投骰子
    分数接近的时候可以多投几个
    不然就投max_scoring_num_rolls个
    """
    # BEGIN PROBLEM 12
    if free_bacon(opponent_score) >= 8:
        return 0
    if pig_pass(score + free_bacon(opponent_score), opponent_score) or swine_align(score + free_bacon(opponent_score), opponent_score):
        return 0
